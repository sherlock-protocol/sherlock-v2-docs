...
2. **Could Denial-of-Service (DOS), griefing, or locking of contracts count as Medium (or High) severity issue?** To judge the severity we use two separate criteria:
   ...
   > Note: if the single occurrence of the attack is relatively long (e.g. >2 days), and it takes only 2-3 iterations cause a 7-day DOS, it may be considered a valid finding.
