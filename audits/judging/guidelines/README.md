...
2. **EIP Compliance:** For issues related to EIP compliance, the protocol & codebase must show that there are important external integrations that would require strong compliance with the EIP's implemented in the code. The EIP must be in regular use or in the **final state** and the report has to qualify for Medium and/or High severiy definitions from the judging guidelines for EIP implementation issues to be considered valid
